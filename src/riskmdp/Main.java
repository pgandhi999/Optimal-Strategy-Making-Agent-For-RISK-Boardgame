...
public class Main
{
	...
	public static long[] optimalPolicy;
	...
	public static void main( String[] args )
	{
		...
		main.buildStateSpaceGraph( objective, player );
		optimalPolicy = new long[states.size()];
		main.computeTheBestPossibleStrategy();
		main.parseOptimalPolicyAndPrintOutput();
	}
	...
	private void buildStateSpaceGraph( String objective, String player )
	{
		if( objective.equalsIgnoreCase( objectives.CONTROL_EUROPE.toString() ) )
		{
			buildStateSpaceGraphForControlContinent( player, "Europe", 7, 23, 29 );
		}
		if( objective.equalsIgnoreCase( objectives.CONTROL_ASIA.toString() ) )
		{
			buildStateSpaceGraphForControlContinent( player, "Asia", 12, 30, 41 );
		}
		if( objective.equalsIgnoreCase( objectives.CONTROL_NORTH_AMERICA.toString() ) )
		{
			buildStateSpaceGraphForControlContinent( player, "North America", 9, 14, 22 );
		}
		if( objective.equalsIgnoreCase( objectives.CONTROL_AUSTRALIA.toString() ) )
		{
			buildStateSpaceGraphForControlContinent( player, "Australia", 4, 0, 3 );
		}
		if( objective.equalsIgnoreCase( objectives.CONTROL_AFRICA.toString() ) )
		{
			buildStateSpaceGraphForControlContinent( player, "Africa", 6, 8, 13 );
		}
		if( objective.equalsIgnoreCase( objectives.CONTROL_SOUTH_AMERICA.toString() ) )
		{
			buildStateSpaceGraphForControlContinent( player, "South America", 4, 4, 7 );
		}
	}
	 ...
	 */
	private void buildStateSpaceGraphForControlContinent( String player, String continent, int numTerr, int startId, int endId )
	{
		try
		{
			...
			State s2 = null;
			State s3 = null;
			ArrayList< Short > actionList = new ArrayList< Short >();
			...
			while( !bfs.isEmpty() )
			{
				...
				if( actions1 > -1 )
				{
					...
					while( true )
					{
						int ps = st1.getParentState();
						if( ps == -1 )
						{
							break;
						}
						actionList.add( st1.getActions() );
						st1 = states.get( ps );
					}
					for( int i = actionList.size() - 1; i >= 0; i-- )
					{
						short act = actionList.get( i );
				...
				for( int i = 0; i < terr1.size(); i++ )
				{
					...
					for( int j = 0; j < adjTerrPlayer.size(); j++ )
					{
						...
						s2.setParentState( ( int ) sid );
						states.add( s2 );
						states.get( ( int ) sid ).getAdjStates().add( iCnt );
						bfs.add( s2 );
						...
						s3.setId( ++iCnt );
						s3.setActions( atr1 );
						s3.setReward( states.get( ( int ) sid ).getReward() - 5 );
						s3.setAdjStates( new ArrayList< Long >() );
						s3.setProbability( 2d );
						s3.setUtility( 0d );
						s3.setParentState( ( int ) sid );
						states.add( s3 );
						states.get( ( int ) sid ).getAdjStates().add( iCnt );
						bfs.add( s3 );
					}
	 ...
	 */
	private void computeTheBestPossibleStrategy()
	{
		State st = null;
		double delta = 1d;
		/*
		...
		while( delta > 0.00000001 )
		{
			...
			for( int i = 0; i < states.size(); i++ )
			{
				...
				double maxProd = 0d;
				long actionState = -2;
				double oldUtility = st.getUtility();
				for( int j = 0; j < st.getAdjStates().size(); j += 2 )
				{
					double p = states.get( ( int ) st.getAdjStates().get( j ).longValue() ).getProbability();
					double u = states.get( ( int ) st.getAdjStates().get( j ).longValue() ).getUtility();
					double prod = p * u + ( ( ( double ) ( 1 - p ) ) * states.get( ( int ) st.getAdjStates().get( j + 1 ).longValue() ).getUtility() );
					if( prod >= maxProd || j == 0 )
					{
						maxProd = prod;
						actionState = st.getAdjStates().get( j );
					}
				}
				double newUtility = 0d;
				if( st.getAdjStates().size() > 0 )
				{
					newUtility = ( double ) st.getReward() + gamma * maxProd;
					optimalPolicy[( int ) st.getId()] = actionState;
				}
				else
				{
					newUtility = ( double ) st.getReward();
					optimalPolicy[( int ) st.getId()] = -1;
				}
				st.setUtility( newUtility );
				if( Math.abs( newUtility - oldUtility ) > delta )
				{
					delta = Math.abs( newUtility - oldUtility );
				}
			}
		}
	}
	 ...
	 */
	private void parseOptimalPolicyAndPrintOutput()
	{
		int i = 0;
		while( optimalPolicy[i] != -1 )
		{
			State st = states.get( ( int ) optimalPolicy[i] );
			short action = st.getActions();
			int from = ( int ) ( action / divisor );
			int to = ( int ) ( action % divisor );
			System.out.println( "Optimal move will be to attack from " + listTerr.get( from ).getCountry() + " to " + listTerr.get( to ).getCountry() );
			System.out.println( "Was the attack a success?" );
		...
		System.out.println( "I hope I was of some help here!" );
	}
}
